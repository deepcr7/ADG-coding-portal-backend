...
async function loginFunction(req,res,next) {
  ...
  const checkpassword = await bcrypt.compare(req.body.password, emailAlreadyExists.password)
  if(!checkpassword){
    res.status(403).send({
  ...
  const token = jwt.sign({_id:emailAlreadyExists.id},process.env.JWT_KEY)
